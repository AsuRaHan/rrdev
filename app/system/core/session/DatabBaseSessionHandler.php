...
class DatabBaseSessionHandler implements \SessionHandlerInterface
{
    ...
    private $model;
    private $SessionTableName = 'sessions';

    public function open($savePath, $sessionName): bool
    {
        $this->model = new Model($this->SessionTableName);
        $sql = "CHECK TABLE {$this->SessionTableName}";
...
CREATE UNIQUE INDEX idx_sessions_id ON {$this->SessionTableName} (session_Id);";
...
//        $sql = "SELECT Session_Data FROM Session WHERE Session_Id = ':id' AND Session_Expires > ':se'";
        $sql = "SELECT session_Data FROM {$this->SessionTableName} WHERE Session_Id = '" . $id . "' AND Session_Expires > '" . date('Y-m-d H:i:s') . "'";
...
//            dd($row['Session_Data']);
    ...
    public function write($id, $data): bool
    {
        ...
        $sql = "REPLACE INTO {$this->SessionTableName} SET Session_Id = '" . $id . "', Session_Expires = '" . $NewDateTime . "', Session_Data = '" . $data . "'";
...
//        dd($result);
    ...
    {
        $sql = "DELETE FROM {$this->SessionTableName} WHERE ((UNIX_TIMESTAMP(Session_Expires) + " . $maxlifetime . ") < " . $maxlifetime . ")";
        $result = $this->model->exec($sql);
        if ($result) {
            return true;
        } else {
