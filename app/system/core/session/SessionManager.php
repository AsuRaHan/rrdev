...
class SessionManager {
    ...
    public static function GarbageCollector($maxlifetime = 7200) {
        ...
        $sesDir = Config::get('session.directory');
        foreach (glob($sesDir . DS . 'sess_*') as $file) {
    ...
    private static function SecSessionStart() {
        ini_set("session.gc_probability", Config::get('session.gc_probability')); /* Можно настроить на 100%, если у вас там нет никакого медленного кода */
        ini_set("session.gc_divisor", Config::get('session.gc_divisor'));
        ini_set("session.gc_maxlifetime", Config::get('session.gc_maxlifetime')); /* Время жизни сессии в секундах (то самое, которое передается в функцию gc) */
        ...
        switch (strtolower(Config::get('session.driver') )) {
            case "database":
